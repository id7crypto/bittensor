...
from bittensor import (
    Subtensor,
    bittensor,
)  # Replace 'your_module' with the actual module name where Subtensor is defined
...
def mock_config():
    class MockConfig:
        def __init__(self):
            self.subtensor = {"chain_endpoint": None, "network": None, "__is_set": {}}
...
@pytest.mark.parametrize(
    "network, config_updates, expected_network, expected_endpoint",
    [
        ("finney", {}, "finney", "ws://finney.network"),
        (None, {"subtensor": {"network": "test"}}, "test", "ws://test.finney.network"),
        (
            None,
            {"subtensor": {"chain_endpoint": "http://localhost:9944"}},
            "local",
            "ws://localhost:9944",
        ),
        (
            None,
            {
                "__is_set": {"subtensor.network": True},
...
def test_setup_config(
    network, config_updates, expected_network, expected_endpoint, mock_config
):
